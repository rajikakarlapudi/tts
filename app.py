...
async def tts(text, voice, speed, output_file):
    # Ensure the rate is in the correct format (+10%, -10%, etc.)
    if speed > 0:
        rate_str = f"+{speed}%"
    elif speed < 0:
        rate_str = f"{speed}%"
    else:
        rate_str = "0%"  # Default to 0% if no speed change

    print(f"Using rate: {rate_str}")  # Log the rate being used
...
def infer_tts_audio(tts_voice, tts_text, tts_speed):
    ...
    # Convert tts_speed from string to integer
    speed_value = int(tts_speed.rstrip('%'))  # Extract the numerical value from the percentage string
